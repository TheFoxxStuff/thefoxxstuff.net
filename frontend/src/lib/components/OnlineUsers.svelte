<script>
  /**
   * OnlineUsers — виджет онлайн пользователей.
   *
   * Props:
   *   users     — [{user_id, username, display_name, avatar_thumb, role}]
   *   count     — число онлайн
   *   compact   — true = только аватарки стопкой, false = развёрнутый список
   *   maxShow   — сколько аватарок показывать (остальные +N)
   */
  import { API_BASE } from '$lib/api';

  let { users = [], count = 0, compact = false, maxShow = 7 } = $props();
...
{#if count > 0}
  {#if compact}
    ...
    <div class="flex items-center gap-2">
      ...
      <span class="text-xs text-[--w40] flex items-center gap-1">
  ...
  {:else}
    ...
    <div class="space-y-1">
      <div class="flex items-center gap-2 mb-3">
        ...
        <span class="text-sm text-[--w60]">{count} online right now</span>
      </div>
      {#each visible as user}
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <div class="text-sm text-white truncate group-hover:text-accent-green transition-colors">
      ...
      {/each}
      {#if rest > 0}
        <p class="text-xs text-[--w30] px-2 pt-1">+{rest} more</p>
      {/if}
    </div>
  {/if}
{/if}
