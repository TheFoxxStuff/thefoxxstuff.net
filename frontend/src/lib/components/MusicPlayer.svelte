            ...
            >
              {#if showVolume}
                ...
                >
                  ...
                  <div class="relative w-1 h-[90px] mb-1">
                    <div class="absolute w-full h-full bg-white/[0.06] rounded-full"></div>
                    <div class="absolute bottom-0 w-full bg-white rounded-full" style="height:{state.volume * 100}%"></div>
                    <div class="absolute w-[11px] h-[11px] bg-white rounded-full left-1/2 -translate-x-1/2 shadow-[0_0_8px_rgba(0,0,0,0.8)] pointer-events-none"
              ...
              {/if}
              <button class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/[0.06] text-white/45 hover:text-white transition-all" aria-label="Громкость">
                {#if state.volume === 0}
